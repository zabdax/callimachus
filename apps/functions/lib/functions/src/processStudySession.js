...
async function innerHandler(request) {
    ...
    if (!last.empty) {
        const endedAtMs = last.docs[0].data().endedAtMs;
...
export const processStudySession = onCall(innerHandler);
// Test hook: allow unit tests to call `processStudySession.run(data, ctx)` directly.
processStudySession.run = (data, ctx) => innerHandler({ data, auth: ctx.auth, rawRequest: ctx.rawRequest });
