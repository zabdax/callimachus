 ...
 */
export function buildRevisionReminder(input) {
    const safeName = input.chapterName.length > 60
 ...
 */
export const sendRevisionReminder = onSchedule({ schedule: 'every 1 hours', timeZone: 'Asia/Dhaka' }, async () => {
    const now = Date.now();
    ...
    const sends = snap.docs.map((d) => {
        ...
        return admin.messaging().send(buildRevisionReminder(data));
    });
