...
const day = 86400_000;
const TYPES = ['firstRevision', 'secondRevision', 'thirdRevision'];
const OFFSETS = {
    firstRevision: 7,
    secondRevision: 14,
    thirdRevision: 30,
};
export const scheduledRevisions = onDocumentWritten('users/{uid}/syllabus/{subjectId}', async (event) => {
    ...
    for (const [chapterId, afterCh] of Object.entries(after)) {
        ...
        const justFirstStudy = !prev.firstStudy && !!afterCh.firstStudy && !!afterCh.firstStudyDate;
        if (!justFirstStudy)
            continue;
        const firstDate = afterCh.firstStudyDate.toDate();
